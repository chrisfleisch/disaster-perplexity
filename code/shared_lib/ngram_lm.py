...
class AddKTrigramLM(object):
    ...
    def __init__(self, tokens):
        ...
        self.context_totals = {}
        for context_key, context_value in self.counts.iteritems():
            # context_value is a dict, sum all it's values
            context_total = sum(context_value.itervalues())
            self.context_totals[context_key] = context_total
    ...
    def next_word_proba(self, word, seq):
        ...
        abc_count = self.counts.get(context, 0.0)
        if abc_count != 0:
            abc_count = self.counts[context].get(word, 0.0)
        ab_count = self.context_totals.get(context, 0.0)
        
        prob = (abc_count + float(k)) / (ab_count + float(k) * float(self.V))
        return prob
        #### END(YOUR CODE) ####
...
class KNTrigramLM(object):
    ...
    def __init__(self, tokens):
        ...
        for word in tokens:
            #### YOUR CODE HERE ####
            self.counts[()][word] += 1
            if w_1 is not None and w_2 is None:
        ...
        for context, words_dict in self.counts.iteritems():
            self.context_totals[context] = float(sum(words_dict.values()))
        # Count the number of nonzero entries for each context.
        for context, words_dict in self.counts.iteritems():
            self.context_nnz[context] = len(words_dict)
        # Compute type fertilities, and the sum z_tf.
        for key, value in self.type_contexts.iteritems():
            # for each word store the length of the set of words that precedes it
            self.type_fertility[key] = len(value)
    ...
    def kn_interp(self, word, context, delta, pw):
        ...
        if self.context_totals.get(context, 0) != 0:
            # calculate Alpha_context
            alpha = float(delta) / float(self.context_totals.get(context, 0)) \
                * math.ceil(float(self.context_nnz.get(context, 0)) - float(delta))
            
            # calculate P_kn(w | context)
            prob = max(0., (float(self.counts[context].get(word, 0)) - float(delta))) \
                    / float(self.context_totals.get(context, 0))
                
            # P_kn(w | context) + alpha * pw
            prob += (alpha * pw)
            return prob
        else:
            return pw # unkown context, return backoff prob
        #### END(YOUR CODE) ####
